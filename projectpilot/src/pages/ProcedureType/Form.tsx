...
import { createProcedureTypeModel, type ProcedureTypeModel } from "../../BussinessLogic/Models/ProcedureType.Model";
...
export const ProcedureTypeForm = ({ isEdit, initialValues, loadData }: Props) => {
    ...
    const { mutate: createOrUpdate } = useMutationApi<boolean, ProcedureTypeModel>((procedure) => procedureTypesService.CreateOrUpdate(procedure!), {
        optimisticUpdate: () => {
            handleReset();
            loadData();
        },
        onSuccess: () => {
            ...
            setSubmitSuccess(`Procedure Type has been ${operation} successfully!`);
            setSubmitError(null);
        },
        onError: () => {
            const operation = isEdit ? "updated" : "created";
            setSubmitSuccess(null);
            setSubmitError(`Procedure Type could not be ${operation}!`);
        },
    });

    const onSubmit = async (data: CreateFormData) => {
        const raw = isEdit ? { id: data.id, name: data.name } : { name: data.name };
        const procedureType = createProcedureTypeModel(raw);

        await createOrUpdate(procedureType);
